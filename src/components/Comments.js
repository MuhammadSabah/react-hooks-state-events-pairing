...
function Comments({ data, onCommentClick }) {
    ...
    if (comment.id === 1) {
      return (
        <div key={comment.id}>
          ...
          <p>{comment.comment}</p>
          <button
            className="firstUpVotes"
            type="button"
            onClick={() =>
              setVotes({ ...votes, firstUpVote: votes.firstUpVote + 1 })
            }
          >
            {votes.firstUpVote} 👍
          </button>
          <button
            className="firstDownVotes"
            type="button"
            onClick={() =>
              setVotes({ ...votes, firstDownVote: votes.firstDownVote + 1 })
            }
          >
            {votes.firstDownVote} 👎
          </button>{" "}
      ...
      return (
        <div key={comment.id}>
          ...
          <p>{comment.comment}</p>
          <button
            className="secondUpVotes"
            type="button"
            onClick={() =>
              setVotes({ ...votes, secondUpVote: votes.secondUpVote + 1 })
            }
          >
            {votes.secondUpVote} 👍
          </button>
          <button
            className="secondDownVotes"
            type="button"
            onClick={() =>
              setVotes({ ...votes, secondDownVote: votes.secondDownVote + 1 })
            }
          >
            {votes.secondDownVote} 👎
          </button>{" "}
          <br />
          <button
            className="seconedRemove"
            type="button"
            onClick={() => handleRemoveComment(comment.id)}
          >
            Remove Comment
  ...
  return (
    <div>
      ...
      <br />
      {onCommentClick && (
        <button className="sort" onClick={() => filteredNames.sort()}>
          Sort Comments
        </button>
      )}
